    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="px-4 md:px-6 py-8 md:py-10">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            <p className="text-orange-500 font-semibold mb-2 text-base">
              Kitchen Workflow
            </p>

            <h1 className="text-3xl md:text-3xl md:text-5xl font-extrabold leading-tight">
              Live Orders
            </h1>

            <div className="mt-4 bg-zinc-950 border border-white/10 rounded-2xl p-4 flex flex-wrap gap-3 text-sm font-bold">
              <span className="text-yellow-400">1. Pending</span>
              <span className="text-gray-500">→</span>
              <span className="text-cyan-400">2. Confirm</span>
              <span className="text-gray-500">→</span>
              <span className="text-blue-400">3. Preparing</span>
              <span className="text-gray-500">→</span>
              <span className="text-green-400">4. Ready</span>
            </div>
          </div>

          <div className="grid grid-cols-2 xl:grid-cols-5 gap-3 md:gap-5 mb-6">
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-4 md:p-6">
            ...
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-4 md:p-6">
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold text-blue-400">
                {analytics.preparing}
              </h2>
            </div>
            ...
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-4 md:p-6">
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold text-green-400">
                {analytics.ready}
              </h2>
            </div>
            ...
            <div className="bg-zinc-950 border border-white/10 rounded-2xl p-4 md:p-6">
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold text-orange-500">
          ...
          {!loading && orders.length > 0 && (
            <div className="grid xl:grid-cols-2 gap-5">
              {orders.map((order) => {
                ...
                <div
                  key={order.id}
                  className={`bg-zinc-950 rounded-3xl p-5 md:p-7 transition ${
                    isNewOrder
                ...
                >
                  ...
                  <div className="space-y-3 mb-6">
                    {order.items.map((item) => (
                      <div
                        key={item.id}
                        className="bg-black border border-white/10 rounded-2xl p-4 flex items-center justify-between gap-4"
                      >
                        <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {order.status === "PENDING" && (
                      <button
                        ...
                        className="flex-1 min-w-[180px] bg-cyan-500 hover:bg-cyan-600 disabled:opacity-50 text-white py-5 rounded-2xl font-extrabold text-lg transition"
                      >
                        Confirm Order
                      </button>
                    )}
                    ...
                    {order.status === "CONFIRMED" && (
                      <button
                        ...
                        disabled={updatingOrderId === order.id}
                        className="flex-1 min-w-[180px] bg-blue-500 hover:bg-blue-600 disabled:opacity-50 text-white py-5 rounded-2xl font-extrabold text-lg transition"
                      >
                        Start Preparing
                      </button>
                    )}
                    ...
                    {order.status === "PREPARING" && (
                      <button
                        ...
                        disabled={updatingOrderId === order.id}
                        className="flex-1 min-w-[180px] bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white py-5 rounded-2xl font-extrabold text-lg transition"
                      >
                        Mark Ready
                      </button>
                    )}
