...
export const resendVerificationOtp = async (emailData) => {
  const result = await apiRequest("/auth/resend-verification-otp", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export const resendResetOtp = async (emailData) => {
  const result = await apiRequest("/auth/resend-reset-otp", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const PRODUCTS_CACHE_DURATION = 5 * 60 * 1000
...
const PUBLIC_SETTINGS_CACHE_DURATION = 10 * 60 * 1000
...
export const getPublicSettings = async () => {
  ...
  if (
    publicSettingsCache &&
    now - publicSettingsCacheTime < PUBLIC_SETTINGS_CACHE_DURATION
  ) {
  ...
  publicSettingsRequest = apiRequest("/settings/public")
    .then((settings) => {
      publicSettingsCache = settings
      publicSettingsCacheTime = Date.now()
      return settings
    })
    .finally(() => {
